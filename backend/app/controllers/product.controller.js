...
const getCountRatings = asyncHandler(async (req, res) => {
  ...
  const oneStarCount = await Product.countDocuments({ 
    totalRatings: { $gte: 0, $lte: 1 },
    ...dateFilter
  });
  const twoStarCount = await Product.countDocuments({ 
    totalRatings: { $gte: 1, $lte: 2 },
    ...dateFilter
  });
  const threeStarCount = await Product.countDocuments({ 
    totalRatings: { $gte: 2, $lte: 3 },
    ...dateFilter
  });
  const fourStartCount = await Product.countDocuments({ 
    totalRatings: { $gte: 3, $lte: 4 },
    ...dateFilter
  });
  const fiveStartCount = await Product.countDocuments({ 
    totalRatings: { $gte: 4, $lte: 5 },
    ...dateFilter
  });
  ...
  const result = {
    ...
    '4 sao': fourStartCount,
    '5 sao': fiveStartCount,
  };

...
const createVnPayOrder = asyncHandler(async(req, res) => {
    ...
    // Setup VNPay payment URL
    process.env.TZ = 'Asia/Ho_Chi_Minh';
    
    let date = new Date();
    // FIX: Dùng date thay vì response.createdAt (vì response chưa được định nghĩa)
    let createDate = moment(date).format('YYYYMMDDHHmmss');
    let ipAddr = req.headers['x-forwarded-for']?.split(',').shift() 
    ...
    vnp_Params['vnp_Amount'] = Math.round(total * 100);

...
from src.config.config import settings
from src.config.connectMongoDB import inventories_collection, products_collection, user_collection, order_collection, receipt_collection
...
async def chat(request: ChatRequest):
    ...
    if round(confidence, 4) > 0.5:
        ...
        if tag == 'ton_kho_thap':
            low_inventory = inventories_collection.find({
                "currentStock": {"$lt": 160},
                "type": "product"
            })
            inventories = list(low_inventory)

            if inventories:
                list_items = []
                for product in inventories:
                    try:
                        product_doc = products_collection.find_one({"_id": product['productId']})
                        product_name = product_doc['name_vn'] if product_doc else "Không rõ tên"
                        list_items.append(f"<li>{product_name} ({product['currentStock']} sản phẩm)</li>")
                    except Exception:
                        list_items.append(f"<li>ID: {product.get('productId', 'Unknown')} ({product['currentStock']} sản phẩm)</li>")
                headers = [
        ...
        if tag == 'ton_kho_cao':
            high_inventory = inventories_collection.find({
                "currentStock": {"$gt": 160},
                "type": "product"
            })
            inventories = list(high_inventory)
            if inventories:
                list_items = []
                for product in inventories:
                    try:
        ...
        if tag == 'truy_van_ton_kho':
            ...
            matched_products = list(products_collection.find(
                {"name_vn": {"$regex": pattern, "$options": "i"}}
            ))

            if matched_products:
                product_ids = [product['_id'] for product in matched_products]
                inventory_list = list(inventories_collection.find({
                    "productId": {"$in": product_ids}
                }))
                inventory_dict = {inv['productId']: inv for inv in inventory_list}

                results = []
                for product in matched_products:
                    product_id = product['_id']
                    product_name = product['name_vn']
                    if product_id in inventory_dict:
                        stock = inventory_dict[product_id]['currentStock']
                        ...
                        results.append({
                            'name': product_name,
                            'stock': 0,
                            'status': "❌ Không có thông tin tồn kho"
                        })
        ...
        if tag == 'truy_van_luot_mua_tat_ca':
            ...
            if products:
                response_list = []
                for product in products:
                    response_list.append(f"{product['name_vn']} có lượt mua ({product['sold']} lượt mua sản phẩm)")
                headers = [
        ...
        if tag == 'luot_mua_thap':
            ...
            if products:
                response_list = []
                for product in products:
                    response_list.append(f"<li>Sản phẩm {product['name_vn']} có lượt mua là {product['sold']}</li>")
                headers = [
        ...
        if tag == 'truy_van_danh_gia_tat_ca':
            ...
            if products:
                response_list = []
                for product in products:
                    response_list.append(f"<li>{product['name_vn']} có đánh giá là ({product['totalRatings']} sao)</li>")
                headers = [
        ...
        if tag == 'danh_gia_cao':
            ...
            if products:
                ...
                return JSONResponse(content={"response": random.choice(no_data_responses)})
        # Truy vấn trạng thái đang bán của sản phẩm
        if tag == 'san_pham_dang_ban':
            products = list(products_collection.find({"isActive": "Đang bán"}))
            if products:
                response_list = []
                for product in products:
                    response_list.append(f"<li>{product['name_vn']} có đánh giá là ({product['totalRatings']} sao)</li>")
                headers = [
                    "<p><strong>Các sản phẩm đang bán lần lượt là:</strong></p>",
                    "<p><strong>Dưới đây là danh sách sản phẩm đang bán:</strong></p>",
                    "<p><strong>Thông tin các sản phẩm đang bán:</strong></p>",
                    "<p><strong>Các sản phẩm đang bán, bạn có thể xem chi tiết dưới đây:</strong></p>",
                    "<p><strong>Tôi cung cấp cho bạn thông tin về các sản phẩm đang bán hiện nay:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
                ...
                no_data_responses = [
                    "<p>Hiện tại không có sản phẩm nào đang được bán trong cửa hàng Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</p>",
                    "<p>Các sản phẩm trong cửa hàng đang được bán, tôi chưa tìm thấy. Nếu có câu hỏi, bạn có thể hỏi?</p>",
                    "<p>Không tìm thấy các sản phẩm đang được bán. Ngoài ra, bạn còn câu hỏi nào khác không?</p>",
                    "<p>Chưa có sản phẩm nào được bán. Nếu có câu hỏi nào khác, bạn có thể hỏi tôi.</p>"
                ]
                return JSONResponse(content={"response": random.choice(no_data_responses)})
        # Sản phẩm ngưng bán
        if tag == 'san_pham_ngung_ban':
            products = list(products_collection.find({"isActive": "Ngừng bán"}))
            if products:
                response_list = []
                for product in products:
                    response_list.append(f"<li>{product['name_vn']} có đánh giá là ({product['totalRatings']} sao)</li>")
                headers = [
                    "<p><strong>Các sản phẩm ngưng bán lần lượt là:</strong></p>",
                    "<p><strong>Dưới đây là danh sách sản phẩm ngưng bán:</strong></p>",
                    "<p><strong>Thông tin các sản phẩm ngưng bán:</strong></p>",
                    "<p><strong>Các sản phẩm ngưng bán, bạn có thể xem chi tiết dưới đây:</strong></p>",
                    "<p><strong>Tôi cung cấp cho bạn thông tin về các sản phẩm ngưng bán hiện nay:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
                ...
                no_data_responses = [
                    "<p>Hiện tại không có sản phẩm nào đang ngưng bán trong cửa hàng Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</p>",
                    "<p>Các sản phẩm trong cửa hàng đang ngưng bán, tôi chưa tìm thấy. Nếu có câu hỏi, bạn có thể hỏi?</p>",
                    "<p>Không tìm thấy các sản phẩm đang ngưng bán. Ngoài ra, bạn còn câu hỏi nào khác không?</p>",
                    "<p>Chưa có sản phẩm nào ngưng bán. Nếu có câu hỏi nào khác, bạn có thể hỏi tôi.</p>"
                ]
                return JSONResponse(content={"response": random.choice(no_data_responses)})
        # Hiển thị số lượng khác khách hàng
        if tag == 'so_luong_nguoi_dung':
            users = list(user_collection.find({"role": "2000"})) 
        ...
        if tag == 'so_luong_nhan_vien_quan_ly':
            users = list(user_collection.find({"role": {"$in": ["2002", "2004", "2006"]}})) 
            users_count = len(users)
            if users_count:
                headers = [
        ...
        if tag == 'khach_hang_tiem_nang':
            users = list(user_collection.find({}, {"_id": 1}))
            user_ids = [user["_id"] for user in users]
            orders = list(order_collection.find({"orderBy": {"$in": user_ids}}, {"orderBy": 1}))
            user_ids_with_orders = list(set(order["orderBy"] for order in orders))
            users_with_orders = list(user_collection.find(
                {"_id": {"$in": user_ids_with_orders}},
                {"name": 1}
            ))
            customer_names = [user.get("name", "Không rõ tên") for user in users_with_orders]
            users_count = len(customer_names)
            if users_count:
                headers = [
                    "<p><strong>Danh sách khách hàng tiềm năng đã từng đặt hàng như sau:</strong></p>",
                    "<p><strong>Thông tin khách hàng tiềm năng hiện có:</strong></p>",
                    "<p><strong>Dưới đây là các khách hàng đã từng mua hàng:</strong></p>",
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
                ...
                no_data_responses = [
                    "<p>Hiện không có khách hàng nào đã từng đặt đơn hàng. Bạn còn câu hỏi nào không?</p>",
                    "<p>Tôi chưa tìm thấy khách hàng tiềm năng nào trong hệ thống. Nếu có câu hỏi, bạn có thể hỏi?</p>",
                    "<p>Không tìm thấy khách hàng tiềm năng nào. Ngoài ra, bạn còn câu hỏi nào khác không?</p>",
                ]
        ...
        if tag == 'khach_chua_mua_hang':
            users = list(user_collection.find({"role": "2000"}, {"_id": 1, "name": 1}))
            user_ids = [user["_id"] for user in users]
            orders = list(order_collection.find({"orderBy": {"$in": user_ids}}, {"orderBy": 1}))
        ...
        if tag == 'tim_kiem_nhan_vien_theo_ten':
            ...
            matched_users = list(user_collection.find(
                {"name": {"$regex": pattern, "$options": "i"}, "role": {"$in": ['2002','2004', '2006']}},
                {"_id": 1, "name": 1, "email": 1, "phone": 1, "address": 1}
            ))
        ...
        if tag == 'tim_kiem_nhan_vien_theo_email':
            ...
            matched_users = list(user_collection.find(
                {"email": email, "role": {"$in": ['2002','2004', '2006']}},
                {"_id": 1, "name": 1, "email": 1, "phone": 1, "address": 1}
            ))
            headers = [
        ...
        if tag == 'tim_kiem_nhan_vien_theo_sdt':
            ...
            matched_users = list(user_collection.find(
                {"phone": phone, "role": {"$in": ['2002','2004', '2006']}},
                {"_id": 1, "name": 1, "email": 1, "phone": 1, "address": 1}
            ))
            headers = [
        ...
        if tag == 'tim_kiem_nhan_vien_theo_dia_chi':
            ...
            matched_users = list(user_collection.find({
                "$or": [
                    {"address.province.name": {"$regex": re.escape(address_query), "$options": "i"}, "role": {"$in": ['2002','2004', '2006']}},
                    {"address.district.name": {"$regex": re.escape(address_query), "$options": "i"}, "role": {"$in": ['2002','2004', '2006']}},
                    {"address.ward.name": {"$regex": re.escape(address_query), "$options": "i"}, "role": {"$in": ['2002','2004', '2006']}},
                    {"address.detail": {"$regex": re.escape(address_query), "$options": "i"}, "role": {"$in": ['2002','2004', '2006']}},
                    {"address.addressAdd": {"$regex": re.escape(address_query), "$options": "i"}, "role": {"$in": ['2002','2004', '2006']}},
                ]
            }, {"_id": 1, "name": 1, "email": 1, "phone": 1, "address": 1}))
            headers = [
        ...
        if tag == 'don_hang_dang_xu_ly':
            ...
            return JSONResponse(content={"response": response_html})    
        # chi tiết đơn hàng đơn hàng đang xử lý
        if tag == 'chi_tiet_don_hang_dang_xu_ly':
            ...
            for i, order in enumerate(order_pending, 1):
                ...
                response_html += f"<ul>"
                response_html += f"<li><strong>Mã đơn:</strong> {order.get('code', 'N/A')}</li>"

                # Khách hàng
                order_by_id = order.get('orderBy')
                if order_by_id:
                    try:
                        customer = user_collection.find_one({"_id": ObjectId(order_by_id)})
                        if customer:
                            customer_name = customer.get('name', customer.get('username', 'N/A'))
                        ...
                        response_html += f"<li><strong>Khách hàng:</strong> Lỗi truy vấn</li>"
                else:
                    response_html += f"<li><strong>Khách hàng:</strong> N/A</li>"

                response_html += f"<li><strong>Tổng tiền:</strong> {order.get('total', 0):,} VNĐ</li>"
                response_html += f"<li><strong>Phương thức thanh toán:</strong> {order.get('paymentMethod', 'N/A')}</li>"
                response_html += f"<li><strong>Trạng thái thanh toán:</strong> {order.get('paymentStatus', 'N/A')}</li>"
                ...
                note = order.get('note', '')
                if note:
                    response_html += f"<li><strong>Ghi chú:</strong> {note}</li>"
                ...
                if products:
                    ...
                    for j, product in enumerate(products, 1):
                        ...
                        response_html += (
                            f"<li>{j}. {product_name} - SL: {quantity} | Giá: {price:,} VNĐ | Thành tiền: {total_product:,} VNĐ</li>"
                        )
                    response_html += "</ul></li>"
        ...
        if tag == 'chi_tiet_don_hang_da_nhan':
            ...
            if not order_count:
                return JSONResponse(content={"response": "<p><strong>Không tìm thấy đơn hàng đã nhận hoặc đã thanh toán. Nếu còn câu hỏi khác, xin vui lòng hãy nhập thông tin câu hỏi cụ thể? Cảm ơn bạn!!!.</strong></p>"})

            response_html = f"<p><strong>Tìm thấy {order_count} đơn hàng trong trạng thái đã nhận hoặc đã thanh toán:</strong></p><hr/>"
        ...
        if tag == 'chi_tiet_don_hang_da_huy':
            ...
            order_count = len(order_pending)
            if not order_count:
                return JSONResponse(content={"response": "<p><strong>Không tìm thấy đơn hàng đã hủy.</strong></p>"})

            response_html = f"<p><strong>Tìm thấy {order_count} đơn hàng trong trạng thái đã hủy:</strong></p><hr/>"
            ...
            return JSONResponse(content={"response": response_html})
        # Thông tin nhân viên có danh thu 
        if tag == 'nhan_vien_doanh_thu_cao':
            try:
                orders = list(order_collection.find({
                    "staff": {"$exists": True, "$ne": None},
                    "status": {"$in": ["Completed", "Processing", "Delivering"]}
                }))
                if not orders:
                    return JSONResponse(content={"response": "Không tìm thấy đơn hàng nào có thông tin nhân viên."})
                # Tính doanh thu theo từng nhân viên
                revenue_by_staff = {}
                for order in orders:
                    staff_id = order["staff"]
                    total = order.get("total", 0)
                    revenue_by_staff[staff_id] = revenue_by_staff.get(staff_id, 0) + total

                # Lọc nhân viên có doanh thu dưới 20 triệu
                low_revenue_staff = {k: v for k, v in revenue_by_staff.items() if v >= 20000000}
                print(low_revenue_staff)

                if not low_revenue_staff:
                    response = [
                        "Không có nhân viên nào có doanh thu cao",
                        "Chưa ghi nhận doanh thu cao ở nhân viên",
                        "Chưa tìm thấy thông tin nhân viên có doanh thu cao"
                    ]
                    return JSONResponse(content={"response": random.choice(response)})

                # Lấy thông tin nhân viên từ user_collection
                staff_ids = list(low_revenue_staff.keys())  
                staff_object_ids = [ObjectId(id) for id in staff_ids]

                staffs = list(user_collection.find(
                    {"_id": {"$in": staff_object_ids}},
                    {"_id": 1, "name": 1}
                ))
                headers = [
                    "Dưới đây là danh sách nhân viên có doanh thu cao:",
                    "Sau đây là thống kê về các nhân viên có doanh số bán hàng cao:",
                    "Danh sách nhân viên chưa đạt chỉ tiêu doanh số:",
                ]
                footers = [
                    "Bạn có muốn tôi hỗ trợ gì thêm không?",
                    "Bạn có thể hỏi thêm bất kỳ câu hỏi nào nhé!",
                    "Nếu cần hỗ trợ thêm, đừng ngần ngại hỏi tôi.",
                    "Bạn còn muốn xem thống kê nào khác không?",
                ]
                header_html = random.choice(headers)
                footer_html = random.choice(footers)
                list_html = ""
                for staff in staffs:
                    ...
                    name = staff.get("name", "Không rõ tên")
                    revenue = low_revenue_staff.get(staff_id, 0)
                    list_html += f"<li><strong>{name}</strong>: {revenue:,} VNĐ</li>"

                # Tạo phản hồi dạng HTML hoàn chỉnh
                response_html = f"""
                <p>{header_html}</p>
                <ul>
                    {list_html}
                ...
                return JSONResponse(content={"response": f"Đã xảy ra lỗi: {str(e)}"})
        # Thông tin nhân viên có danh thu 
        if tag == 'nhan_vien_doanh_thu_thap':
            try:
                orders = list(order_collection.find({
                    "staff": {"$exists": True, "$ne": None},
                    "status": {"$in": ["Completed", "Processing", "Delivering"]}
                ...
                if not low_revenue_staff:
                    response = [
                        "Không có nhân viên nào có doanh thu thấp",
                        "Chưa ghi nhận doanh thu thấp ở nhân viên",
                        "Chưa tìm thấy thông tin nhân viên có doanh thu thấp"
                    ]
                    return JSONResponse(content={"response": random.choice(response)})
                ...
                headers = [
                    "Dưới đây là danh sách nhân viên có doanh thu thấp:",
                    "Sau đây là thống kê về các nhân viên có doanh số bán hàng thấp:",
                    "Danh sách nhân viên chưa đạt chỉ tiêu doanh số:",
                ]
                footers = [
                    "Bạn có muốn tôi hỗ trợ gì thêm không?",
                ...
                return JSONResponse(content={"response": f"Đã xảy ra lỗi: {str(e)}"})
        # thông tin chi tiết doanh thu của nhân viên
        if tag == 'chi_tiet_doanh_thu_nhan_vien':
            try:
                name_patterns = [
                r"nhân viên tên là ([\w\s]+)",
                r"nhân viên ([\w\s]+)",
                r"doanh thu nhân viên tên là ([\w\s]+)", 
                r"doanh thu nhân viên tên ([\w\s]+)", 
                r"nhan viên tên ([\w\s]+)",
                r"cua ([\w\s]+)",
                ]
                staff_name = extract_value(name_patterns, message)
                
                # Tìm nhân viên theo tên
                staff = user_collection.find_one({
                    "$or": [
                        {"name": {"$regex": staff_name, "$options": "i"}},
                        {"username": {"$regex": staff_name, "$options": "i"}}
                    ]
                })
                
                if not staff:
                    return JSONResponse(content={"response": f"Không tìm thấy nhân viên với tên: {staff_name}"})
                
                staff_id = str(staff['_id'])
                
                # Lấy tất cả đơn hàng của nhân viên này
                staff_orders = list(order_collection.find({
                    "staff": staff_id,
                    "status": {"$in": ["Completed", "Processing", "Delivering"]}
                }).sort("createdAt", -1))
                
                if not staff_orders:
                    return JSONResponse(content={"response": f"Nhân viên {staff.get('name', staff_name)} chưa có đơn hàng nào"})
                
                # Tính toán thống kê
                total_revenue = sum(order.get('total', 0) for order in staff_orders)
                order_count = len(staff_orders)
                avg_order_value = total_revenue // order_count if order_count > 0 else 0
                
                response = f"<h3>👤 CHI TIẾT DOANH THU NHÂN VIÊN: {staff.get('name', staff_name)}</h3>"
                response += "<h4>📊 TỔNG QUAN:</h4>"
                response += f"<p>• 💰 <strong>Tổng doanh thu:</strong> {total_revenue:,} VNĐ</p>"
                response += f"<p>• 📦 <strong>Tổng đơn hàng:</strong> {order_count}</p>"
                response += f"<p>• 📈 <strong>Doanh thu trung bình/đơn:</strong> {avg_order_value:,} VNĐ</p>"
                response += "<h4>📋 CHI TIẾT CÁC ĐƠN HÀNG:</h4>"
                for i, order in enumerate(staff_orders[:10], 1):
                    status_mapping = {
                    ...
                    payments = order.get('paymentMethod', 'N/A')
                    payments_status = order.get('paymentStatus', 'N/A')
                    status_vn = status_mapping.get(status, status)
                    payments_vn = payment_methods.get(payments, payments)
                    payments_status_vn = payment_statuses.get(payments_status, payments_status)
                    
                    response += f"<div style='border:1px solid #ccc; padding:10px; margin-bottom:15px;'>"
                    response += f"<h5>{i}. 📦 <strong>{order.get('code', 'N/A')}</strong></h5>"
                    response += f"<p>💰 <strong>Giá trị:</strong> {order.get('total', 0):,} VNĐ</p>"
                    response += f"<p>📋 <strong>Trạng thái:</strong> {status_vn}</p>"
                    response += f"<p>💳 <strong>Thanh toán:</strong> {payments_vn}</p>"
                    response += f"<p>💳 <strong>Trạng thái thanh toán:</strong> {payments_status_vn}</p>"
                    
                    created_at = order.get('createdAt', '')
                    if created_at:
                        try:
                            created_date = datetime.fromisoformat(str(created_at).replace('Z', '+00:00'))
                            response += f"<p>📅 <strong>Ngày tạo:</strong> {created_date.strftime('%d/%m/%Y %H:%M')}</p>"
                        except:
                            response += f"<p>📅 <strong>Ngày tạo:</strong> {created_at}</p>"
                    # Thêm chi tiết sản phẩm
                    products = order.get('products', [])
                    if products:
                        response += "<ul><strong>🛒 Danh sách sản phẩm:</strong>"
                        for product in products:
                            name = product.get("name", "N/A")
                            quantity = product.get("quantity", 0)
                            price = product.get("price", 0)
                            price_type = product.get("priceType", "N/A")
                            thumb = product.get("thumb", "")

                            response += f"<li style='margin-top:10px;'>"
                            if thumb:
                                response += f"<img src='{thumb}' alt='thumbnail' width='80' style='vertical-align:middle; margin-right:10px;'/>"
                            response += f"<strong>{name}</strong><br/>"
                            response += f"Số lượng: {quantity} | Giá: {price:,} VNĐ | Loại giá: {price_type}"
                            response += "</li>"
                        response += "</ul>"

                    response += "</div>"

                if len(staff_orders) > 10:
                    response += f"<p>... và {len(staff_orders) - 10} đơn hàng khác</p>"

                response += "<p><em>Bạn cần xem thêm thông tin gì về nhân viên này không?</em></p>"
                return JSONResponse(content={"response": response})
                
            except Exception as e:
                return JSONResponse(content={"response": f"Lỗi xử lý dữ liệu: {str(e)}"})
        # Doanh thu theo ngày
        if tag == 'doanh_thu_theo_ngay':
            try:
                ...
                # Query đơn hàng trong ngày cụ thể
                orders = list(order_collection.find({
                    "status": {"$in": ["Completed", "Processing", "Delivering"]},
                    "createdAt": { 
                        "$gte":start_of_day,  
                ...
                # Query phiếu xuất trong ngày cụ thể
                receipts = list(receipt_collection.find({
                ...
                if not orders and not receipts:
                    response = [
                        ...
                        f"Chưa có thông tin về doanh thu trong ngày {formatted_date}. Đồng nghĩa với việc có đơn hàng và phiếu xuất nào được tạo. Nếu có câu hỏi khác, tôi sẵn sàng hỗ trợ bạn.",
                ...
                def create_detailed_response():
                        ...
                        for idx, order in enumerate(orders, 1):
                            ...
                            <div style="border: 1px solid #ddd; margin-bottom: 15px; border-radius: 4px;">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                    <h4 style="margin: 0; color: #2c3e50;">#{order.get('code', 'N/A')}</h4>
                                ...
                                <p style="margin: 5px 0; color: #7f8c8d;">💳 Thanh toán: {payment_statuses.get(order.get('paymentStatus', 'N/A'),order.get('paymentStatus', 'N/A'))}</p>
                    ...
                    if receipts:
                        ...
                        for idx, receipt in enumerate(receipts, 1):
                            ...
                            <div style="border: 1px solid #ddd; margin-bottom: 15px; border-radius: 4px;">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                    <h4 style="margin: 0; color: #2c3e50;">#{receipt.get('code', 'N/A')}</h4>
                                    <span style="background: {status_color}; color: white; padding: 4px 8px; border-radius: 12px; font-size: 12px;">
                                        {receipt_statues.get(receipt.get('status', 'N/A') ,receipt.get('status', 'N/A'))}
                                    </span>
                                </div>
                                <p style="margin: 5px 0; color: #7f8c8d;">⏰ Thời gian: {receipt_time}</p>
                                <p style="margin: 5px 0; color: #7f8c8d;">💳 Thanh toán: {payment_statuses.get(receipt.get('paymentStatus', 'N/A'),receipt.get('paymentStatus', 'N/A'))}</p>
                            ...
                            if materials:
                                ...
                                for material in materials:
                                    material_name = material.get('name', 'Sản phẩm không rõ tên')
                                    ...
                                    html_content += f"""
                                    <div style="margin-bottom: 8px;  background: white; border-radius: 3px;">
                                        <div style="font-weight: bold; color: #2c3e50; margin-bottom: 3px;">{material_name}</div>
                                        <div style="font-size: 14px; color: #7f8c8d;">
                                            Số lượng: {quantity} × {price:,} VNĐ = <strong style="color: #e74c3c;">{subtotal:,} VNĐ</strong>
                                        </div>
                                    </div>
                ...
                if show_detail:
                    ...
                    responses = [
                        f"Doanh thu ngày {formatted_date} là: {total_revenue:,} VNĐ. Trong đó doanh thu từ đơn hàng là {total_order_revenue:,} VNĐ, doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Ngoài ra, bạn còn câu hỏi nào khác không?",
                        f"Tổng doanh thu ngày {formatted_date}: {total_revenue:,} VNĐ. Doanh thu từ đơn hàng là {total_order_revenue:,}. Doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Bạn cần tôi giải đáp gì nữa không?",
                        (
                            ...
                            f"</ul>"
                            f"<p>Bạn có muốn xem chi tiết đơn hàng không? Chỉ cần nói 'doanh thu chi tiết ngày...'</p>"
                        )
                    ]
        ...
        if tag == 'doanh_thu_theo_thang':
            try:
                ...
                # Query đơn hàng trong tháng cụ thể
                orders = list(order_collection.find({
                    "status": {"$in": ["Completed", "Processing", "Delivering"]},
                    "createdAt": { 
                        "$gte": start_of_month,
                ...
                # Query phiếu xuất trong tháng cụ thể
                receipts = list(receipt_collection.find({
                    "status": {"$in": ["pending", "confirmed"]},
                    "typeReceipt": "export",
                    "createdAt": { 
                ...
                if show_detail:
                    ...
                    # Hiển thị thông tin tổng quan
                    responses = [
                        f"Doanh thu tháng {formatted_month} là: {total_revenue:,} VNĐ. Trong đó doanh thu từ đơn hàng là {total_order_revenue:,} VNĐ, doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Ngoài ra, bạn còn câu hỏi nào khác không?",
                        f"Tổng doanh thu tháng {formatted_month}: {total_revenue:,} VNĐ. Doanh thu từ đơn hàng là {total_order_revenue:,} VNĐ. Doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Bạn cần tôi giải đáp gì nữa không?",
                        (
                            ...
                            f"<li><strong>Tổng cộng: {total_revenue:,} VNĐ</strong></li>"
                            f"</ul>"
                            f"<p>Bạn có muốn xem chi tiết doanh thu theo khách không? Ví dụ: thông tin chi tiết doanh thu tháng ...</p>"
                        )
                    ]
                    response = random.choice(responses)
        ...
        if tag == 'doanh_thu_theo_nam':
            try:
                ...
                if show_detail:
                    ...
                    responses = [
                        f"Doanh thu năm {target_year} là: {total_revenue:,} VNĐ. Trong đó doanh thu từ đơn hàng là {total_order_revenue:,} VNĐ, doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Trung bình mỗi tháng là {avg_monthly_revenue:,.0f} VNĐ. Ngoài ra, bạn còn câu hỏi nào khác không?",
                        f"Tổng doanh thu năm {target_year}: {total_revenue:,} VNĐ. Doanh thu từ đơn hàng là {total_order_revenue:,} VNĐ. Doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Bạn cần tôi giải đáp gì nữa không?",
                        (
        ...
        if tag == 'doanh_thu_khoang_thoi_gian_theo_ngay':
            try:
                ...
                if not target_start_date and target_end_date:
                    error_responses = [
                        "Không thể hiểu định dạng ngày. Vui lòng nhập theo format: DD/MM/YYYY, DD/MM hoặc 'hôm nay'",
                        "Format ngày không đúng. Bạn có thể nhập như: 15/7, 15/7/2025, hôm nay, hôm qua",
                        "Tôi không hiểu ngày bạn muốn xem. Vui lòng thử lại với format: DD/MM hoặc DD/MM/YYYY"
                ...
                end_of_day = datetime.combine(target_end_date, datetime.min.time())
                ...
                if not orders and not receipts:
                    response = [
                        f"Không tìm thấy bất kỳ khoản doanh thu nào từ ngày {formatted_start_date} đến này {formatted_end_date}. Bạn còn câu hỏi nào khác không?",
                        f"Từ ngày {formatted_start_date} đến này {formatted_end_date} chưa xuất hiện bất kỳ khoản doanh thu nào. Nếu có câu hỏi khác, tôi sẵn sàng hỗ trợ bạn.",
                        f"Doanh thu từ ngày {formatted_start_date} đến này {formatted_end_date} là 0 VNĐ. Bạn có thể truy vấn doanh thu theo khoản thời gian khác nếu muốn."
                    ]
                    return JSONResponse(content={"response": random.choice(response)})
                ...
                if show_detail:
                    ...
                    responses = [
                        f"Doanh từ ngày {formatted_start_date} đến này {formatted_end_date} là: {total_revenue:,} VNĐ. Trong đó doanh thu từ đơn hàng là {total_order_revenue:,} VNĐ, doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Ngoài ra, bạn còn câu hỏi nào khác không?",
                        f"Tổng doanh thu từ ngày {formatted_start_date} đến này {formatted_end_date}: {total_revenue:,} VNĐ. Doanh thu từ đơn hàng là {total_order_revenue:,} VNĐ. Doanh thu từ phiếu xuất là {total_receipt_revenue:,} VNĐ. Bạn cần tôi giải đáp gì nữa không?",
                        (
                            f"<p>Doanh thu từ ngày {formatted_start_date} đến này {formatted_end_date}:</p>"
                            f"<ul>"
                            f"<li>Doanh thu đơn hàng: {total_order_revenue:,} VNĐ ({len(orders)} đơn)</li>"
                            f"<li>Doanh thu phiếu xuất sản phẩm: {total_receipt_revenue:,} VNĐ ({len(receipts)} phiếu)</li>"
                            f"<li><strong>Tổng cộng: {total_revenue:,} VNĐ</strong></li>"
...
# ------ Mô hình chatbot sử dụng Neutural ------- #
# Đọc intents từ tệp intents.json
...
async def chat(request: ChatRequest):
    ...
    if prob.item() > 0.7:
            ...
            if matched_products:
                ...
                if len(results) == 1:
                    ...
                    headers = [
                        ...
                        "<p>Tôi tìm ra được 1 sản phẩm</p>",
                        "<p>Với từ khóa, tôi tìm được 1 sản phẩm</p>",
                        "<p>Đây là thông tin sản phẩm</p>"
                    ]
                    footers = [
                        "<p>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</p>",
        ...
        if tag == 'truy_van_luot_mua_tat_ca':
            ...
            if products:
                ...
                headers = [
                    ...
                    "<p>Dưới đây là thông tin lượt mua của các sản phẩm:</p>",
                    "<p>Doanh số các sản phẩm bán ra, bạn có thể tham khảo:</p>",
                    "<p>Sau đây là thông tin về lượt mua của các sản phẩm tại hệ thống:</p>",
                    "<p>Rất sẵn lòng, tôi sẽ cung cấp thông tin về lượt của các sản phẩm dưới đây:</p>",
                ]
                response = "Các sản phẩm có lượt mua lần lượt là: " + ", ".join(response_list)
                return JSONResponse(content={"response": response})
            else:
                response = "Hiện tại không có sản phẩm nào được mua. Nếu có câu hỏi khác, tôi sẽ sẵn sàng trả lời bạn"
                return JSONResponse(content={"response": response})
        # Lượt mua thấp
        if tag == 'luot_mua_thap':
            products = list(products_collection.find({"sold": {"$lt": 100}}))
            if products:
                ...
                headers = [
                    ...
                    "<p><strong>Tôi đã liệt kê các sản phẩm có lượt mua thấp, dưới đây:</strong></p>",
                    "<p><strong>Về lượt mua thấp, dưới đây là các sản phẩm:</strong></p>",
                    "<p><strong>Rất sẵn lòng, tôi sẽ cung cấp cho bạn thông tin về các sản phẩm có lượt mua thấp tại hệ thống:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
        ...
        if tag == 'luot_mua_cao':
            ...
            if products:
                ...
                headers = [
                    ...
                    "<p><strong>Tôi đã liệt kê các sản phẩm có lượt mua cao, dưới đây:</strong></p>",
                    "<p><strong>Dưới đây là các sản phẩm có lượt mua cao:</strong></p>",
                    "<p><strong>Okie, tôi sẽ cung cấp cho bạn thông tin của các sản phẩm có lượt mua cao sau đây:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
        ...
        if tag == 'truy_van_danh_gia':
            ...
            name_query = extract_value(name_patterns, message)
            if not name_query:
                return JSONResponse(content={"response": "Bạn vui lòng cung cấp tên sản phẩm cần tìm. Tôi sẽ hỗ trợ bạn truy vấn lượt đánh giá ngay lập tức."})
            keywords = name_query.strip().lower().split()
            pattern = "(?=.*" + ")(?=.*".join(map(re.escape, keywords)) + ")"
            ...
            if len(results) == 1:
                ...
                headers = [
                        ...
                        f"<p>Tôi tìm thấy {len(results)} sản phẩm liên quan đến từ khóa '<em>{(name_query)}</em>'.</p>",
                        f"<p>Với từ khóa bạn cung cấp, tôi tìm được {len(results)} sản phẩm sau:</p>",
                        f"<p>Okie bạn, tôi tìm được {len(results)} sản phẩm như sau:</p>",
                    ]
                footers = [
                    "<p>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</p>",
                    "<p>Nếu có câu hỏi, bạn có thể hỏi?</p>",
        ...
        if tag == 'truy_van_danh_gia_tat_ca':
            ...
            if products:
                ...
                headers = [
                    ...
                    "<p><strong>Bạn có thể tham khảo về lượt đánh giá của các sản phẩm:</strong></p>",
                    "<p><strong>Tôi cung cấp cho bạn thông tin về lượt đánh giá của tất cả sản phẩm:</strong></p>",
                    "<p><strong>Rất sẵn lòng, tôi sẽ cung cấp cho bạn thông tin về lượt đánh giá như sau:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
        ...
        if tag == 'danh_gia_thap':
            ...
            if products:
                ...
                headers = [
                    ...
                    "<p><strong>Các sản phẩm có lượt đánh giá thấp, bạn có thể xem chi tiết dưới đây:</strong></p>",
                    "<p><strong>Tôi cung cấp cho bạn thông tin về lượt đánh giá thấp của các sản phẩm:</strong></p>",
                    "<p><strong>Với câu hỏi bạn đưa ra, tôi sẽ cung cấp thông tin về những sản phẩm có lượt đánh giá thấp, cụ thể như sau:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
        ...
        if tag == 'danh_gia_cao':
            ...
            if products:
                ...
                headers = [
                    ...
                    "<p><strong>Các sản phẩm có lượt đánh giá cao, bạn có thể xem chi tiết dưới đây:</strong></p>",
                    "<p><strong>Tôi cung cấp cho bạn thông tin về lượt đánh giá cao của các sản phẩm:</strong></p>",
                    "<p><strong>Bạn cần thông tin của những sản phẩm có lượt đánh giá cao, tôi sẽ hiển thị chi tiết cho bạn như sau:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
        ...
        if tag == 'san_pham_dang_ban':
            ...
            if products:
                ...
                headers = [
                    ...
                    "<p><strong>Các sản phẩm đang bán, bạn có thể xem chi tiết dưới đây:</strong></p>",
                    "<p><strong>Tôi cung cấp cho bạn thông tin về các sản phẩm đang bán hiện nay:</strong></p>",
                    "<p><strong>Hiện đối với các sản phẩm đang bán tại hệ thống, tôi sẽ cung cấp thông tin như sau:</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
        ...
        if tag == 'san_pham_ngung_ban':
            ...
            if products:
                ...
                headers = [
                    ...
                    "<p><strong>Các sản phẩm ngưng bán, bạn có thể xem chi tiết dưới đây:</strong></p>",
                    "<p><strong>Tôi cung cấp cho bạn thông tin về các sản phẩm ngưng bán hiện nay:</strong></p>",
                    "<p><strong>Cảm ơn bạn đã đặt câu hỏi, tôi sẽ cung cấp thông tin của các sản phẩm đang ngưng bán tại hệ thống hiện nay.</strong></p>"
                ]
                footers = [
                    "<p><strong>Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</strong></p>",
                    "<p><strong>Nếu có câu hỏi, bạn có thể hỏi?</strong></p>",
        ...
        if tag == 'so_luong_nguoi_dung':
            ...
            if users_count:
                ...
                header_text = random.choice(headers)
                footer_text = random.choice(footers)
                response_html = f"<p>{header_text}{users_count} tài khoản đã đăng ký.{footer_text}</p>"
                return JSONResponse(content={"response": response_html})
            else:
                no_data_responses = [
                    "<p>Hiện tại không có người dùng nào trong hệ thống. Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</p>",
        ...
        if tag == 'so_luong_nhan_vien_quan_ly':
            ...
            if users_count:
                ...
                header_html = random.choice(headers)
                footer_html = random.choice(footers)
                response_html = f"<p>{header_html}{users_count}.{footer_html}</p>"
                return JSONResponse(content={"response": response_html})
            else:
                no_data_responses = [
                    "<p>Hiện tại không có tài khoản nào trong hệ thống. Bạn còn câu hỏi nào không? Tôi luôn sẵn sàng hỗ trợ.</p>",
    ...
    return JSONResponse(content={"response": "Tôi không hiểu bạn nói gì. Tôi được thiết lập để trả lời các câu hỏi liên quan đến hoạt động, tình hình kinh doanh cũng như doanh thu của cửa hàng."})
